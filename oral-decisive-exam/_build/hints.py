...
BLOCKS = [
    ...
    ("台大", "戴浩志", "s29",
     "台大這一關是<strong>戴浩志理事長</strong>。⚠ <strong>只確認人選，沒有給任何出題方向。</strong>",
     "同學回報",
     "⚠ <strong>這一關的情報只有「誰」，沒有「考什麼」</strong>——所以不能縮小範圍，只能照手冊的既有檔案準備。"
     "好消息是手冊對他的記錄很完整：他是 <strong>114 年 15 站裡出題最密集的一關</strong>，"
     "用一張全喉咽切除的照片問滿六題，當場指名 Neligan 第五版第三卷第十三章。"
     "⭐ 他的主軸是<strong>頭頸重建</strong>，而且要的是「先描述缺損 → 界定目標 → 選血管 → 選皮瓣」的骨架。",
     [("下咽環周缺損：描述、目標、三瓣比較（ALT／radial forearm／jejunum）",
       [("戴浩志那一關", "s29")], "✅ 手冊已有"),
      ("受區血管的選擇與 frozen neck",
       [("戴浩志那一關", "s29")], "✅ 手冊已有"),
      ("Free flap 術後監測、失敗與搶救",
       [("Flap 監測與搶救", "tm91")], "✅ 手冊已有"),
      ("⭐ 語音課 EP09 整集就是這一關（頭頸重建與皮瓣原則，9 題）", [], "🎧 語音課"),
      ("⚠ 沒有出題提示，所以<strong>範圍不能縮</strong>——顯微重建全線都要備", [], "📌 現場執行")]),
    ...
    ("北榮", "彭成康", "s30",
     ...
     [("ISSVA 分類要能秒答（KTS ＝ CLVM，PIK3CA）",
      ...
      ("KTS 的組成、與 Parkes Weber 的分界、併發症與治療（含那條被課本推翻的紅線）",
       [("Klippel–Trénaunay", "tb207")], "✅ 已補寫"),
    ...
    ("高醫", "郭耀仁", None,
     ...
     "⭐ 查醫院官網補上：他<strong>已由長庚轉任高醫</strong>，現為高醫整形外科教授，"
     "專長除了顯微重建與手外科，還有<strong>複合組織異體移植</strong>——2014 年完成台灣首例、"
     "亞洲第一例手臂移植。這條線與林承弘的 VCA 那一關重疊。"
...
INTRO = """
<p><strong>這一節只收一種東西：確定出場的老師，以及他們自己說出口的提示。</strong>不是考古題統計、不是我的推測——
...
其餘每一條都可以直接點進去讀。十關裡<strong>只有簡守信那一關是「背熟就會過」</strong>；<strong>台大戴浩志只確認了人、沒有給方向，範圍不能縮</strong>；其餘八關給的是<strong>範圍</strong>不是題目，所以要備滿。</p>
