...
ROWS = [
    ...
    ("未具名・和藹的長輩<br>（疑似 高雄長庚　江原正）", "🟢 推論吻合",
     "特徵四項全中，照江原正那一節準備",
     "和藹的長輩，<strong>聽力稍差要大聲一點回答</strong>；專長手外科、"
     "<strong>congenital hand anomalies、hand tumors</strong>；情境題為主；"
     "也曾經考過<strong>請考生畫一隻手</strong>接著自由出題。〔寧・8/26〕",
     "⭐ 特徵四項全中<strong>江原正</strong>那一節："
     "次專長就是先天手（足）畸形 ＋ skin tumor（melanoma、NF、hemangioma、DFSP）；"
     "手冊也記了他 108 年考過「<strong>畫手找錯</strong>」。",
     "以江原正那一節準備。三條贏法：<strong>拿到照片先完整描述、最後才給診斷</strong>（診斷猜錯不致命，"
     "描述不完整才致命）；PE 不要漏掉 arm 與 upper arm；完全沒概念時<strong>問考官病人的基本資料</strong>。"
     "⚠ 這是我的推論，同學沒說是誰，<strong>能問到是哪一家醫院最好</strong>。"),
...
<h3>怎麼用這一節</h3>
<ol>
<li><strong>先看「判定」那一欄。</strong>🟢 的四關（陳理維、簡守信、曾元生、疑似江原正）
...
<li>⚠ 這一節全部是<strong>傳聞</strong>。如果同學後續有更新（尤其林政輝的出題方向、
以及那位未具名考官到底是誰），以新的為準。</li>
</ol>
"""
